...
let myTaskObj = [
    {
        ...
        "status": "ToDo",
        "priority": "High",
        "created" : getData.getDate()+"/"+[getData.getMonth()+1]+"/"+getData.getFullYear()
    },
    {
        "taskId":2,
...
function openDes(e){
    let myHtml, myTarget;
    e.stopPropagation();
    myTarget = Number(e.target.id);
    // console.log(myTaskObj[(myTarget-1)].taskName);
    let desBoxDiv = document.getElementById("desBoxId");
    desBoxDiv.innerHTML = "";
    let newDiv = document.createElement("div");
    myHtml = `<h3>-:${myTaskObj[(myTarget-1)].taskName}:-</h3>
              <hr>
              <div class='desBox'>
              <p><b>Status :</b>${myTaskObj[myTarget-1].status}</p>
              <p><b>Priority :</b>${myTaskObj[myTarget-1].priority}</p>
              <p><b>Created :</b>${myTaskObj[myTarget-1].created}</p>
              <p><b>Assignee :</b>Kavya</p>
              </div>
              `;
    newDiv.innerHTML = myHtml;
    desBoxDiv.append(newDiv);
}